...
- **THE REAL COST WAS THE ENTRY-SITE ENUMERATION, AND IT WAS UNDER-ESTIMATED TWICE.** The estimate given before implementation was eight sites, then thirteen after reading the code. The true number is **fourteen**: the six frame pushes, `EnterContainer`, a divert landing at offset 0, a forked thread, the external fallback, the two host entry points — and the tunnel-onwards override (`->-> target(args)`), which no `offset: 0` grep finds because it rewrites the *return address* and binds only after the frame pops. Two of the sites (`Flow::invoke_fallback`, `FlowInstance::begin_function_eval`) carried comments explicitly documenting their dependence on the prologue, which is what a convention being load-bearing looks like from the inside.

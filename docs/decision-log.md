...
- **SCOPE:** architectural
- **SUPERSEDES:** "LIR optimization stage; reachability prune is its first pass" (2026-08-06). That ruling's SUBSTANCE stands unchanged — the mount stays universal and unconditional, the shipped artifact carries only what the project reaches, mount-on-demand stays rejected, every transform is a pure function with no iteration-order dependence. Its MECHANISM is replaced.
- **WHAT:** Two things that were one. (1) **Reachability pruning is a compiler emission step**, not an optimizer pass: unconditional, no config, computed over `lir::Program` before codegen. The 2026-08-06 wording — "codegen emits only definitions reachable from the artifact's roots" — already described this. (2) **The optimizer is a post-compile `.inkb` → `.inkb` transform** in a new `brink-opt` crate depending only on `brink-format`, not a `LIR → passes → LIR` stage. The boundary between them: the compiler decides what to ship; the optimizer makes what ships cheaper without changing what it does.

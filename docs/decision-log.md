...
## Line tables are deduplicated at emission, within a scope
- **WHEN:** 2026-09-05
- **PROJECT:** brink
- **SYSTEM:** `brink-codegen-inkb` (`push_line`, `LineKey`, `dedup_suspended`), `brink-format` (`Eq`/`Hash` on `LineContent`, `LinePart`, `SelectKey`, `SlotInfo`) — `docs/intl-spec.md` §"Line-table deduplication", `docs/optimizer-catalogue.md`
- **SCOPE:** moderate
- **WHAT:** A line authored more than once within one scope is one line-table entry, hence one translation unit. `add_line` and `add_template_line` now share `push_line`, which consults a per-scope `(content, slot_info)` index and returns the existing index on a hit. The first occurrence supplies the entry's `source_hash` and `source_location`. Variant runs (#3273) are exempt in both directions: dedup is suspended while a switch is laid out and a run's entries are never registered as targets, so `base + combo` stays valid.
- **RULED — in codegen, not the optimizer.** The catalogue's `dedup line table` entry asked whether entries should be born deduplicated or deduplicated after the fact; the maintainer's call is emission: smaller, earlier, on every compile road including the studio's, and a translator quality-of-life fix as much as an optimization. The optimizer entry is closed as resolved-elsewhere.
- **RULED — within a scope first.** Story-wide merging crosses translator contexts and needs a shared table or cross-scope `EmitLine` references; it is a possible later step, to be measured first. Measured before the change on TheIntercept: 71 within-scope duplicates of 1 066 units, 170 story-wide, almost all short choice labels; after it, 995 units, and the tier1–3 corpus's line tables shrink from 2 815 entries to 2 440 (−13.3%).
- **RULED — merge by default, opt out later.** Identical source text can need different translations at different sites; a per-line author-side marker that makes a line its own unit is the planned escape hatch, and it doubles as the way to give a repeated line its own VO take (`audio_ref`). Not built yet. In-app translation facilities can later offer "split this unit" on the same mechanism.
- **CONSERVATIVE ON `source_location`:** the merged entry keeps its first occurrence's location and the format is unchanged. A provenance list for every site is the recorded follow-up, additive when it comes.
- **WHY IT IS SAFE FOR EXISTING TRANSLATIONS:** `regenerate-xliff` aligns units by `source_hash` (`brink-intl/src/regenerate.rs`, `align_hashes`), not by index, so an existing XLIFF's translation of a now-merged unit carries over and the dropped duplicates are ordinary deletions. Runtime output is unchanged by construction — the runtime reads content by index and equal content is equal content — which the oracle ratchet and the corpus fence both confirm.
...
- **WHEN:** 2026-09-05
- **PROJECT:** brink
- **SYSTEM:** `brink-runtime` (`vm::bind_entry_params`, every container-entry site), `brink-codegen-inkb` (no parameter prologue, positional-slot validation), `brink-format` (`VERSION` 10) — `docs/compiler-spec.md` §"Parameter binding", `docs/format-spec.md`
- **SCOPE:** architectural (the callee half of the calling convention)
- **WHAT:** Codegen no longer emits the leading `DeclareTemp(slot)` run that bound a parameterized container's arguments. The VM binds `param_count` arguments off the value stack into the frame's temp slots `n-1 … 0` at every entry instead. The caller's half is untouched — evaluate left to right, push, then call/divert — and `DeclareTemp` itself remains for `~ temp` declarations.
- **WHY:** the histogram put `Call → DeclareTemp` at 12.0% of crucible's executed instructions and 5.5% of hanoi's, one dispatch per parameter per call, and no peephole can remove it: it is emission, not an instruction pair. This is the first of the three codegen-side shapes the peephole work surfaced (`docs/optimizer-peephole.md` §5).
- **RULED — the full fix, not the function-only one.** The recommendation on the table was to bind at push for functions only, leaving knots, tunnels and threads on the prologue, because the hot path is functions and the other paths have fewer tests. The maintainer asked why three more call sites were worth avoiding; they were not. A split convention would have to be remembered by every future change and encoded in the debugger's prologue logic, and the uniform version makes `prologue_end` *simpler* (a choice target's `ChoiceOutput` is now the only prologue bytecode that exists).
- **THE REAL COST WAS THE ENTRY-SITE ENUMERATION, AND IT WAS UNDER-ESTIMATED TWICE.** The estimate given before implementation was eight sites, then thirteen after reading the code. The true number is **fifteen**: the six frame pushes, `EnterContainer`, a divert landing at offset 0, a forked thread, the external fallback, three host entry points — and the tunnel-onwards override (`->-> target(args)`), which no `offset: 0` grep finds because it rewrites the *return address* and binds only after the frame pops. Fourteen was the count *this entry originally recorded*, and it was wrong too: `begin_function_value_eval` (host-side re-invocation of an opaque function value) was found by the same `debug_assert` net on the first CI run of `bevy-brink`'s `bench-counters` lane, which the local sweep had excluded. Three successive enumerations of this set — eight, thirteen, fourteen — were each under by at least one, and only the runtime assertion ever closed the gap. Two of the sites (`Flow::invoke_fallback`, `FlowInstance::begin_function_eval`) carried comments explicitly documenting their dependence on the prologue, which is what a convention being load-bearing looks like from the inside.
- **THE NET IS WHAT MADE IT SAFE.** A `debug_assert` in the VM's fetch preamble fires when execution stands at offset 0 of a parameterized container with an unbound parameter slot. It caught the tunnel-onwards site on its first run over the corpus (`tier1/diverts/I060-tunnel-onwards-divert-after-with-arg`), named the container, and cost nothing in release. Enumerating by hand had already missed that site twice; the assertion found it in one run. Same lesson as `scripts/check-scripts.mjs` (hand enumeration of network-touching commands went 0-for-3): **mechanically check the invariant, do not enumerate its instances.**
- **THE ASSUMED INVARIANT WAS FALSE, AND THE CORPUS SAID SO.** The first implementation had the VM bind positionally into slots `0 … n-1`, on the strength of `alloc_temps` allocating parameters first — which a `brink-ir` unit test pins, but only for *knot* parameters. A knot and its stitches share one call frame and one temp map, so a **stitch's parameters continue after its knot's**: `= opt(n)` inside `=== outer(m)` has `opt`'s parameter at slot 1. `thread_frame_growth_3561.rs` caught it. The fix is to stop assuming: `ParamMeta` gains a `slot` field (free, since v10 is a bump anyway) and the VM binds into the slot the artifact records. A guard that merely *checked* the assumption would have been a compile error on legal ink.
...
## The per-knot LIR chunk seam is kept for the dependency, not the latency
- **WHEN:** 2026-09-08
- **PROJECT:** brink
- **SYSTEM:** `brink-db` (`lir_knot_chunk_query`, `file_knot_segments_query`, `chunk_lowering_ctx_query`), `brink-ir` (`ChunkLoweringCtx`, `ResolutionLookup::build_shifted`) — `docs/keystroke-inference-cost.md`
- **SCOPE:** moderate
- **WHAT:** Each knot's LIR chunk is lowered from its own segment fragment instead of the whole-file `normalized_stamped_query`, and `chunk_lowering_ctx_query` keeps only range-free inputs. Measured on TheIntercept (32 knots), `lir_knot_chunk_query` executions per edit: append **32 → 1**, mid-file **32 → 18**.
- **RULED — keep it, on correctness grounds, with the latency case explicitly NOT made.** The seam removes a per-knot artifact invalidating project-wide, and it is a prerequisite for ever making the link incremental — you cannot reuse chunks you keep re-lowering. It buys **no measurable compile time**: ~18.7 ms before, ~17.7 ms after, inside a ±2 ms run-to-run spread. The maintainer's call was "keep it, correctness is good"; this entry exists so the absent speedup is not later mistaken for one, and so nobody re-opens the question expecting a number.
- **THE COMPILE IS NOT INVALIDATION-BOUND, AND THAT IS THE REAL FINDING.** On an append the compile now re-executes essentially nothing — one chunk, one of everything else — and still costs ~17.7 ms on a 100 KB story. The cost is the link and codegen, which are O(whole program) every revision regardless of how well invalidation is scoped. Further dependency-scoping work on the chunk side has almost no headroom; the next lever is the link itself.
- **THE MID-FILE RESIDUE OF 18 IS BY DESIGN.** A chunk is not position-free: `Container`/`Stmt`/`Expr` all carry `Provenance` and the debug line tables are built from it, so the fragment is rebased to absolute before lowering and the memo depends on its segment offset. Every knot after the edit shifts. Reaching one needs a rebase over lowered LIR rather than over HIR.
- **BYTE-IDENTITY RESTS ON AN ORDER:** rebase, then stamp, then normalize — the order `assemble_lowered_file` uses. A node the passes synthesize carries the provenance-free `0..0`; rebasing after normalizing would move what must stay put. `normalize_file`'s `$lift` counter was made per-definition for this, pinned over the corpus by `fragment_normalization_parity`.
- **THE BLOCKER WAS A SHIPPED BUG, NOT THE SEAM.** A first attempt was reverted after five brink-dialect cases read Null at runtime. Root cause: `MapLiteral::rebase` shifted only its `ptr`, never its `entries`, and `StructLiteral` the same with its `fields` — the only two `Vec<(A, B)>` fields in the HIR and the only two skipped. Every expression inside a map or struct literal kept its segment-relative range forever. Self-consistent, so nothing caught it: the resolution map is built from the same un-rebased HIR and both agreed at the wrong coordinate. Only something computing a range independently could see it. Fixed, with a whole-class guard (`rebase_shifts_every_range`) that reads the DERIVED `Debug` rendering rather than walking fields, because a hand-written visitor needs the same per-field enumeration that was wrong here.
- **MEASURE THE TARGET BEFORE BUILDING THE FIX.** The seam was built twice before anything measured the compile it was meant to improve. `interaction_cost_over_real_stories`'s compile row is the session's FIRST compile — every chunk cold — so it is blind to this change by construction. `compile_after_one_edit_when_warm` (added here) is the number an author waits on, and it would have said on day one that the compile was link-bound.

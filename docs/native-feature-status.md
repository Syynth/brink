...
| Elements reach `IdeSession` | ✅ | ❌ | #1880 (still OPEN) — `IdeSession::analysis_options` hardcodes `conventions: None`, so `E169` and every compiler-side conventions query (including this section's own explain-match/projection) are unreachable from live typing through the editor path; discovered again independently while building #2113, filed onward as #2308 |

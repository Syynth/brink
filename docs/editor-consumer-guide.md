...
| HIR structural overlay (#454) | `hirOverlayExtension` (auto-wired by `DocumentSessions` from the doc handle): `brink-hir-<kind>` inline marks + `data-*` identity, per-line `data-hir-rails` attributes, and the `brink-hir-rail-gutter` with one `brink-hir-rail brink-hir-rail-<kind>` marker per stacked container; `refreshHirOverlay(view)` / `refreshHirOverlayEffect` re-read the projection without a doc change — `DocumentSessions` fires it on every compile delivery (#494) AND on a view mount that happens after a compile was already delivered (#518), so passive loads paint in either order; hosts with custom wiring fire it from their own compile-complete signal (it must come from the same module instance of `@brink-lang/editor` that built the view — a bundler-duplicated copy produces an effect the field ignores) | **all rail geometry** — the rail markers are geometry-free empty `<span>`s (inline, 0×0), so your CSS must supply display/width/height (not just color) for the bars to be visible | — (headless: brink emits structure + identity, the host styles it) |
...
`brink-divert-standalone` (a `-> target` divert that is not a tunnel call).

Two pieces of *data-driven placement* (computed per-line from weave depth / divert shape, not skin)
ship as taxonomy, never inline styles (#414) — and since the 2026-08-23 "literal whitespace"
ruling, **no default look is shipped for either**: the editor renders the file's own whitespace
as-is, and these hooks exist for hosts that want a styled layout of their own.

- **Weave depth** — choices/gathers at depth > 1 carry `data-depth="N"` (the raw depth number).
  Style `[data-depth="N"]` yourself, or ignore it.
- **Standalone diverts** — carry the `brink-divert-standalone` class (above); previously the
  shipped theme right-aligned these ("screenplay transition"), now a host choice.
...
sigils), `brink-choice-bracket` (the `[...]` choice-suffix bracket), `brink-fold-decl` /

...
2. Resolve glue (same algorithm as today — mark newlines consumed by glue, stitch text across boundaries). A consumed newline takes every whitespace-only part after it up to the glue with it — a `Spring` or whitespace-only text — exactly as ink's `TrimNewlinesFromOutputStream` does (issue #3507): `a` / `{false:x} <>` / `b` prints `ab`, while `{0} <>` / `world` prints `0 world` because content on the line makes the newline non-trailing and the spring's space survives.

...
- **Fork snapshots**: `CallStack` caches the `Arc<[CallFrame]>` its last `snapshot()` built so successive forks (choice creation, `<-`) share one allocation; **every** mutating access — push, pop, materialize, and the `last_mut`/`get_mut` frame borrows a temp write goes through — clears that cache, so a fork always sees the frames as they are now (issue #3528: a temp written after `<- thread` was missing from the next choice's fork, served from the stale cache).
- **Choice selection**: `choose(index)` is a flow-level operation — restores the thread fork, sets execution position, clears pending choices. `index` numbers the **visible** choices contiguously — the `Choice.index` values `Step::Choices` hands out, C#'s `currentChoices[i].index` / `ChooseChoiceIndex(i)` — and is mapped to the pending-choice position internally; an invisible fallback (`+ ->`) never occupies a slot, even when it sits ahead of a visible choice (a thread's fallback merged before the main flow's choices, issue #3527).
- **Host-directed jump**: `choose_path_string(path)` — the equivalent of ink's `Story.ChoosePathString(path)` with its default `resetCallstack: true`. Resolves a knot/stitch path via the program's `address_by_path` table, force-ends the current flow (callstack collapses to a fresh root frame, pending choices cleared, `did_safe_exit` set — mirroring C# `ResetCallstack` → `StoryState.ForceEnd`), then transfers control through the same `goto_target` divert machinery the `Goto` opcodes use, so the jump counts a visit per the target's counting flags exactly like an in-story `-> path` (C# `ChoosePath` → `SetChosenPath` + `VisitChangedContainersDueToDivert`). The Context is untouched: globals and visit/turn counts carry across the jump, and the output buffer/transcript is kept (C# leaves the output stream alone). Errors: unknown path (named in the message); flow parked on an unresolved external (a pending host call is never silently abandoned — brink extension, C# has no pausable externals); in-flight engine→ink function evaluation. A story that has hit `-> END` may be re-entered by jumping, matching C#.
...
There is deliberately **no `Thread` frame type** (issue #3561). `<- target` pushes no call frame at all — matching ink, where a thread divert compiles to a bare `{"->": ...}`. It forks the thread (see [Thread forks](#thread-forks)), re-points the fork's own copy of the innermost frame at the target, and runs there; the target's parameters bind into that frame's temps exactly as a plain `-> target` divert's would. Where the thread's own execution begins is recorded as `Thread::base_depth`, a mark on the thread rather than a frame in its stack: content exhausted with nothing above the mark means the *thread* is done and is popped whole, never unwinding into the parent's frames below it. A `Thread` boundary *frame* used to carry that fact, but a choice raised inside a thread captures the frame in its fork and selecting the choice installs that fork wholesale — so the boundary rode into the main call stack and was never released, one retained frame per turn in the ordinary `<- thread`-as-choice game loop.
...
2. Resolve glue (same algorithm as today — mark newlines consumed by glue, stitch text across boundaries). A consumed newline takes every whitespace-only part after it up to the glue with it — a `Spring` or whitespace-only text — exactly as ink's `TrimNewlinesFromOutputStream` does (issue #3507): `a` / `{false:x} <>` / `b` prints `ab`, while `{0} <>` / `world` prints `0 world` because content on the line makes the newline non-trailing and the spring's space survives. On function return, the function-end trim (`OutputBuffer::trim_function_end`, ink's `TrimWhitespaceFromFunctionEnd`) walks the function's output backwards removing trailing newlines, springs, whitespace-only text and any *value* that renders as whitespace (an empty list, `""`, a `none` — issue #3536: ink stringifies values into the output stream as they are pushed, so an empty interpolation is an inline-whitespace `StringValue` by the time its trim runs, while brink resolves values later and so judges the unresolved `ValueRef` by `OutputPart::is_visible`), and treats glue as transparent — the glue stays, the whitespace beneath it goes — so `{0} <>` at the end of a function glues `0` to the next line (`0a`, issue #3522), the way ink's loop `continue`s past every non-text object. At the other end of a function, while it has produced no non-whitespace output since it was entered, a newline it emits is dropped outright (ink's `functionStartInOutputStream` rule): the VM's `EmitNewline` consults the top call-stack frame's `OutputMark` — the active target's length and capture/fragment depths at call time — and skips the newline when nothing content-bearing has been pushed past the mark on that same target; a string capture or fragment that began inside the function moves the depths and disables the rule (ink's `BeginString` exception), as does a tunnel frame on top (ink reads the top element only; a thread pushes no element in either runtime — issue #3561). So `x{f(f(true))}y`, with `f`'s body starting on a conditional block whose branch begins with a newline, prints `xaatruey` (issue #3519).

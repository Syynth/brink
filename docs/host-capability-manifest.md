...
It sits *beside* the tiers above rather than inside them: it constrains prose
markup, not call sites, and needs no host protocol at all.
...
[`E164`](diagnostics/E164.md) and an undeclared attribute on a declared kind
reports [`E165`](diagnostics/E165.md). Both default to `Warning`, so severity is
configurable: `[lints] E164 = "deny"` makes the vocabulary binding, and
`@[allow(E164)]` / `// brink-disable E164` silence it locally.

Attribute *values* are not checked — span attribute values are static text by
construction, so only the attribute *name* is part of the declared vocabulary.

...
## Markup vocabulary (a second section, not a tier)

The manifest's `markup` key declares the project's **inline markup
vocabulary** — the span kinds `<name attr="v">…</name>` may use, and the
attributes each kind allows (`docs/prose-dialect-spec.md` §4.2, issue #1733;
required attributes, issue #1780/#1997). It sits *beside* the tiers above
rather than inside them: it constrains prose markup, not call sites, and
needs no host protocol at all.

It lives here — and not in the project's element conventions — by §3.4's
authorship test ("co-locate declarations only if hand-authored or generated by
the same source"). Markup vocabulary is **host-authored**, the same author as
`externals`, and can be *generated* from engine code: a text-effect plugin
auto-declaring its tags, the way bindings generate externals. Element
conventions are project-authored and live in the `brink.toml`-referenced
conventions module instead — a different surface with a different author.
...
`externals`/`types`, is never diagnosed for a markup tag. Declaring one or more
span kinds is what turns checking on, at which point an undeclared tag reports
[`E164`](diagnostics/E164.md), an undeclared attribute on a declared kind
reports [`E165`](diagnostics/E165.md), and a declared kind's span omitting one
of its `required` attributes reports [`E173`](diagnostics/E173.md). All three
default to `Warning`, so severity is configurable: `[lints] E164 = "deny"`
makes the vocabulary binding, and `@[allow(E164)]` / `// brink-disable E164`
silence it locally (same for `E165`/`E173`).

Attribute *values* are not checked — span attribute values are static text by
construction, so only the attribute *name* (and, now, whether it is
`required`) is part of the declared vocabulary. The per-attribute record also
carries a reserved, currently-inert slot for a future value type
(`ManifestSpanAttr::ty`) — schema headroom only; issue #1997 explicitly does
**not** implement attribute-value typing, so that slot is deserialized and
round-tripped but never read by any check.
...
| Undeclared markup tag / attribute diagnostic | `markup` section | ✅ |

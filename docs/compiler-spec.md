...
- **Multi-line conditional arms start with a newline:** every arm of a multi-line conditional — the implicit first arm of `{ cond:`, each `- cond:` / `- else:` arm, each switch arm, each block-sequence branch — lowers with a leading `Stmt::EndOfLine`, matching inklecate's `["\n", "^text", "\n", …]` branch containers (issue #3523). The runtime dedupes it whenever output already ends in a newline, which is almost always; it is observable exactly when the content before the block does not end in one — a printing function called in the condition, whose trailing newline the function-end trim removed — and there ink starts the arm on a new line.
...
- **Normalization pass:** `normalize_file()` runs after lowering, lifting inline sequences and inline conditionals within content to block-level `Stmt` nodes. This simplifies LIR lowering and codegen by ensuring only plain text and interpolations appear as inline content parts; block-level control flow is always represented as `Stmt::Conditional` / `Stmt::Sequence`.
  - **Source-order rule (RULED 2026-09-02, issue #3395):** the lift preserves ink's left-to-right evaluation of the line. Every interpolation to the LEFT of the lifted construct — every `{expr}`, whether or not it contains a call, spans included — is first evaluated in source order into a hidden synthetic temp (`TempDecl::synthetic`, named `$lift{n}`; `$` is not an identifier character, so no authored temp can collide), declared immediately before the lifted statement; the prefix clones spliced into every branch read those temps. The construct's condition (or a sequence's visit-count touch / shuffle draw) therefore runs AFTER the prefix's side effects, where ink runs it — `{bump()}{n == 1:yes|no}` prints `1yes`, and `{n}{bump() == 1:yes|no}` prints `0yes`. Each expression still evaluates exactly once; stateful alternatives in the prefix stay shared clones (#3275); the suffix is handled at the next lift level. A read of a temp minted at an outer lift level is never re-hoisted. Codegen gives a synthetic temp's direct-call value the same fragment composition a call in a slot gets (`emit_slot_expr`), so a prefix call that prints keeps its text inline (`a{shout()}{cond:…}` → `aXyes`) instead of emitting it ahead of the line — the one place a literal `~ temp` hoist would have diverged from ink. That composition applies to any slot (or synthetic temp) whose expression *contains* a call, not only one that *is* a call (`Expr::contains_function_call`, issue #3525): `a{f() == "x"}` evaluates inside the slot's fragment, so `f`'s text lands after `a` where ink puts it; a call-free compound slot (`{n + 2}`) is still evaluated bare. The temps are hidden from the debugger via the `DebugInfo` locals row's `synthetic` flag (`docs/debugger-spec.md` §3); translation identity is unaffected (`source_hash` canonicalizes every interpolation to `{…}`), though a hoisted slot's `slot_info` display name reads `$lift{n}` (`docs/intl-spec.md`).
  - **The all-false path owes the line's newline (#3530):** a lifted conditional with no authored else arm gets a synthesized one whenever the line carries prefix text, suffix text, *or its own end-of-line*. The last is not optional: ink suppresses a line's `\n` only when the line produced no content, and a condition that prints is content — so `{f():a}` whose `f` prints and returns false still ends its line. Synthesizing an arm holding only an `EndOfLine` is safe for the silent case, because the runtime drops a newline with no content before it, leaving `{false:a}` emitting nothing.
...
**`Content` and `ContentPart`** — a line of text output with inline elements. Parts include plain text, glue, expression interpolation (`{expr}`), inline conditionals (`{cond: a | b}`), inline sequences (`{&a|b|c}`), and the internal word-break `Spring` — a conditional space the runtime resolves (RULED internal 2026-08-02, no authoring spelling) that lowering mints in two places: a choice's start text ending in whitespace (`replace_trailing_ws_with_spring`), and the whitespace between an inline construct's `}` and `<>` (issue #3507; the lexer folds a space after TEXT into the TEXT token, but after `}` it is trivia the parser skips, so `{0} <>` needs the spring to print ink's `0 world`). Block-level conditionals and sequences are separate `Stmt` variants, not content parts — this reflects the genuine semantic distinction in ink between inline elements (which produce text fragments) and block elements (which contain statements).
...
1. **Merge manifests** (`manifest::merge_manifests`) — merge per-file symbol manifests into a unified `SymbolIndex`. Duplicate declarations emit warnings (E022/E023/E026) matching inklecate's permissive behavior (first-wins semantics). Names that shadow built-in functions emit E035 — see [E035](diagnostics/E035.md) for which symbol kinds shadow at which positions, including the call-site divergence between a shadowing knot/external (always wins) and a shadowing `VAR`/`CONST` (never wins a real call site).
2. **Resolve references** (`resolve::resolve_refs`) — name resolution: unresolved `Path` nodes → concrete `DefinitionId`s. Handles scope analysis (temp is function-scoped, VAR/CONST are global). Produces a `ResolutionMap` mapping source ranges to their resolved definitions. Resolution follows ink's hierarchical scoping: local stitches/labels first, then knots, then top-level, then labels by suffix match. Function call arity is checked (E031). A reference whose name also matches a built-in (the classic uppercase ink intrinsics, or the T1b stdlib names) still resolves to a real declared symbol first when one exists — the builtin-name check is a fallback tried only once every declared-symbol lookup has already failed, never a precondition on doing the lookup at all (issue #2856), per the "rules attach to resolved definitions, never names" ruling below. The reserved-name set itself is defined once, in `brink-ir` (`brink_ir::lir::is_builtin_function`/`is_t1b_stdlib_name`, Pass 4's "Built-in function recognition" below) — `resolve::is_builtin_function`/`is_t1b_stdlib_name` delegate to it rather than keeping their own copy, per the 2026-03-06 decision-log ruling and issue #2863.
...
- **Provenance-carrying (issue #3183).** `Container`, `Stmt`, and `Expr` each carry a bare (non-`Option`) `Provenance` recording the source construct they were lowered from, copied off the originating HIR node's own already-stamped range at lowering time. A node synthesized during a desugar with no single corresponding HIR node (an RMW take/mutate/write-back leg, an auto-inserted `-> DONE`, a goto-gather divert, …) inherits the provenance of the HIR statement it was desugared from via `LowerCtx::current_stmt_provenance`, rather than a fabricated value. `Expr` mirrors `Stmt`'s split: the bare enum is `ExprKind`, and `Expr = { kind: ExprKind, provenance: Provenance }`. Most `hir::Expr` shapes carry no `.ptr` of their own and so also inherit `current_stmt_provenance`; the ~10 that do (`Infix`/`Coalesce`, `ArrayLiteral`, `MapLiteral`, `Index`, `Range`, `StructLiteral`, `FieldAccess`, `FnLiteral`, `Lambda`, `RefArg`) get that tighter range instead — each step of a folded `or`-coalescing chain gets its own originating `Infix` node's range, not the whole chain's. See `docs/sourcemap-epic-evaluation.md` §1.
...
- **Built-in function recognition:** for a call site whose callee failed to resolve to any real declared symbol (`ctx.resolve_path` returns `None`), recognizes names matching ink built-in functions (`TURNS_SINCE`, `LIST_COUNT`, `INT`, `FLOOR`, etc.) as a post-resolution fallback and converts them to `Expr::CallBuiltin` nodes instead of container calls. Resolution wins first: a real declared knot/external/variable/local of the same name is lowered against *that* resolved definition, never against the builtin table, matching Pass 3's "rules attach to resolved definitions, never names" ordering (issue #2856; `docs/decision-log.md`, "Language-facility doctrine: standard functions vs intrinsics vs syntax", 2026-07-12 — "an intrinsic's special typing/lowering rule applies only when resolution lands on the builtin"). **This is where the reserved-name set is defined** (`recognize_builtin`/`is_builtin_function`/`is_t1b_stdlib_name` in `brink_ir::lir::lower::expr`, re-exported as `brink_ir::lir::is_builtin_function`/`is_t1b_stdlib_name`) — `brink-analyzer`'s Pass 3 resolver delegates to these rather than keeping its own copy (2026-03-06 decision-log ruling, delivered by issue #2863).
...
| [`E063`](diagnostics/E063.md) | A param/return/`VAR`/`CONST`/`~ temp` type annotation disagrees with the type TM-1's body inference would otherwise derive, or (issue #1877) a `VAR`/`CONST`/`~ temp` declaration initializer or a plain assignment disagrees with its target's already-known declared type, or (issue #1881) a UFCS-desugared call's receiver or written argument disagrees with the desugared free function's already-known declared param type, or (issue #1918) a UFCS `FieldCall` — a call through a struct's own fn-typed field — is called with the wrong arity or a mismatched argument type against the field's own declared `fn(T…): R` row (no receiver-prepending desugar, unlike the #1881 producer above — the field's value is called directly with the written arguments only), or (issue #2872, `strict::check_value_calls`) a call through a `temp`/param value inside a named knot or stitch has a known (non-`Unknown`, non-`Conflicted`) type that isn't callable, or is called with the wrong arity or a mismatched argument type — this last producer, like the others, fires only under the effective `types = strict` policy — and, unlike the others, only inside named knots/stitches (never the file's root/entry flow, issue #2880). |

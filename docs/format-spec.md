...
- **Path hash** — `i32`, sum of char values from the container's ink path string. Used to seed the RNG for shuffle sequences (ink draws with `sequenceHash + loopIndex + storySeed`).
  - **RULED 2026-09-04 (#3538), NOT YET IMPLEMENTED:** this field is *specified* as ink's hash of ink's path string, and the compiler is to **compute an ink-compatible path string and hash it ink's way — on both surfaces, native included**. Codegen already sums char codes of a path string and already carries three hand-written compatibility rules, but they are heuristics over brink's own container tree rather than a reconstruction of inklecate's. Measured divergence: for `=== k ===` holding a lone sequence and no choice, brink emits `k.0.0` (295) where instrumented inkjs seeds from `k.0` (201) — brink applies its implicit-stitch rule where inklecate emits no such container level. That is a defect against this line, not a re-specification of it.
  - **The constraint is on this derived value, not on the container model.** brink keeps its own container ids, layout and path scheme; it gains the ability to compute a compatible path on demand for the purposes that need one. The compiler's shape is not dictated by inklecate's.
  - **Native is not exempt,** though nothing native needs to agree with inklecate: `brink-respell` re-emits ink as `.brink`, and a respelled story is expected to behave like its original. A surface-dependent seed would make respelling silently change which alternative a shuffle picks.
- **Param count** — `u8`, the number of parameters the container declares (a parameterized knot/stitch/function, e.g. `=== call(action, present) ===` has 2; `0` for the vast majority). **Since v10 this field is load-bearing at every entry, not just for arity checks:** the VM binds that many arguments off the value stack into the frame whenever execution arrives at offset 0 of the container, which is what the leading `DeclareTemp` prologue used to do by sitting there. It binds them into the slots `params` records (`ParamMeta::slot`, new in v10) rather than into `0 … n-1` — a knot and its stitches share one frame and one temp map, so a stitch's parameters continue after its knot's. It also still lets the runtime arity-check a host-directed parameterized entry (`choose_path_string_with_args`) and `call_function`. (Historical: `.inkb` files built by the retired converter reference pipeline left this `0` — inklecate's JSON didn't expose it.)
...
| `EmitLineNl` (`0x6C`) | `u16` line index + `u8` slot count | `EmitLine` immediately followed by `EmitNewline`, fused. **Optimizer-only** (`docs/optimizer-peephole.md`): codegen never emits it; `brink opt`'s peephole pass does, and the runtime executes the two bodies in sequence. v7. |
| `BinaryImm` (`0x6D`) | `u8` `BinaryKind` + `i32` immediate | `PushInt(imm)` followed by the binary operator, fused: pops the left operand, applies the operator with `imm` on the right, pushes the result. **Optimizer-only** (`docs/optimizer-peephole.md` §1). v8. |
| `BinaryJumpIfFalse` (`0x6E`) | `u8` `BinaryKind` + `i32` relative offset | The binary operator followed by `JumpIfFalse`, fused: pops both operands, jumps (relative to the end of this instruction) unless the result is truthy; the result never touches the stack. **Optimizer-only.** v8. |
| `BinaryImmJumpIfFalse` (`0x6F`) | `u8` `BinaryKind` + `i32` immediate + `i32` relative offset | `PushInt(imm)`, the binary operator, then `JumpIfFalse`, fused — the shape of every `if x <= 1` and `{ x == 3: }`. **Optimizer-only.** v8. |
| `GetTempBinaryImm` (`0x70`) | `u16` slot + `u8` `BinaryKind` + `i32` immediate | `GetTemp(slot)`, `PushInt(imm)`, the operator, fused: the temp read is exactly `GetTemp`'s (pointer auto-dereference, #3354 unwritten-slot default). **Optimizer-only** (`docs/optimizer-peephole.md` §1). v9. |
| `GetTempBinaryImmJumpIfFalse` (`0x71`) | `u16` slot + `u8` `BinaryKind` + `i32` immediate + `i32` relative offset | `GetTemp(slot)`, `PushInt(imm)`, the operator, `JumpIfFalse`, fused — every `if n <= 1` over a local. **Optimizer-only.** v9. |
| `DuplicateBinaryImmJumpIfFalse` (`0x74`) | `u8` `BinaryKind` + `i32` immediate + `i32` relative offset | `Duplicate`, `PushInt(imm)`, the operator, `JumpIfFalse`, fused: compares the top of the stack against `imm` without popping it and branches — a switch-style conditional's arm test. **Optimizer-only.** v9. |

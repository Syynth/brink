...
- **Path hash** — `i32`, sum of char values from the container's ink path string. Used to seed the RNG for shuffle sequences (ink draws with `sequenceHash + loopIndex + storySeed`).
  - **RULED 2026-09-04 (#3538), NOT YET IMPLEMENTED:** this field is *specified* as ink's hash of ink's path string, and the compiler is to **compute an ink-compatible path string and hash it ink's way — on both surfaces, native included**. What it emits today is a hash of brink's own container path scheme, which is a different function of a different path, so brink and inklecate pick different shuffle permutations for the same story. That is a defect against this line, not a re-specification of it.

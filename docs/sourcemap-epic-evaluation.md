...
|---|---|---|---|
| 1 | LIR provenance (spans on `lir::Stmt`/`Expr`) | **delivered (issue #3183)** | `lir::Container`, `lir::Stmt`, and now `lir::Expr` all carry a bare (non-`Option`) `Provenance`, reusing B0.1's `Provenance` type verbatim as this row anticipated. `Expr` follows `Stmt`'s `{ kind, provenance }` split (the bare enum renamed `ExprKind`): the ~10 `hir::Expr` shapes that already carry a real `.ptr` of their own (`Infix`/`Coalesce`, `ArrayLiteral`, `MapLiteral`, `Index`, `Range`, `StructLiteral`, `FieldAccess`, `FnLiteral`, `Lambda`, `RefArg`) get that exact range; every other shape inherits `LowerCtx::current_stmt_provenance`, the same statement-level ambient `Stmt`'s own synthesized siblings use. Landed despite the ~700-site blast radius the issue #3183 PR's own scope-cut rationale (previous revision of this row) flagged as a reason to defer — the Rust compiler's exhaustiveness checking made the mechanical part of that migration low-risk; D6 (#3184) can now choose true per-expression granularity for its instruction table rather than being forced to statement granularity. |

...
(`emit-line-nl`), the first superinstruction on the shared peephole engine —
`docs/optimizer-peephole.md`. Consequences for the fence:
...
- `.inkb` moved to **v7** for the new opcode (`0x6C`); fuzz seeds were
  regenerated. Codegen never emits the fused form.

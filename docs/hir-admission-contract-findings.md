...
9. **Builtins not pre-resolved.** `resolve`/`dialect_gate` assume the frontend leaves `len`/`keys`/`none`/`LIST_MIN`… as *unresolved* refs; hardcoded name lists are hand-synced between `resolve.rs:788` and LIR lowering (`dialect_gate.rs:228` `is_t1b_stdlib_call_name`).
10. **`Dialect`/`TypePolicy` are NOT in HIR** — they arrive as `AnalysisOptions` (`lib.rs:75`); a frontend cannot and must not embed them.
11. **Declarations aren't in the shared block-tree walk.** `hir::visit::HirVisitor` deliberately skips the flat file-level decl vecs and tag contents (`hir/visit.rs:19-33`); `dialect_gate`/`structs`/`map_keys`/`conversions` hand-walk `hir.variables`/`hir.structs` initializers separately. A frontend adding a new declaration-position expression must remember every hand-walk site.

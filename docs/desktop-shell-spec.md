...
| `onFileChanged` | **buffer, don't write** | v1 keeps the studio's explicit-save model: dirty state lives in the editor; disk writes happen on `file.save`/`saveAll` via `requestSave`. Autosave ships unconditionally: a 2-minute ticker (120 s) dispatches `file.saveAll` whenever dirty files exist, queuing behind any write already in flight (#2403). This ticker is the production caller both #2435 and #2434's fixes exist to serve correctly |

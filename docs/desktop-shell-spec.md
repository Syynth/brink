...
- **Every job in every workflow file sets an explicit `timeout-minutes`**
  ...
  next `cargo dist generate` regeneration. The allowlist is removable once
  cargo-dist's template supports the field, or a maintainer decides
  hand-editing is acceptable.

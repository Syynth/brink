...
```text
E187: write to a CONST — CONST is immutable and can never be reassigned, mutated, or passed by `ref`: `SPEED` is declared CONST — it cannot be reassigned, mutated, or passed by `ref`
```

/**
 * @brink-lang/dialect — the dialogue-dialect artifact, pure TypeScript.
 *
 * Extracted from `@brink-lang/editor`'s `dialect.ts` (RULED 2026-08-30,
 * "Engines consume the RESOLVED dialect as a compile output"): a game
 * codebase that reads its project's `dialect.json` should not depend on
 * an editor package (CodeMirror and all). The editor re-exports this
 * module verbatim, so its public surface is unchanged; the one
 * editor-coupled helper (`convertibleShapesOf`, which needs
 * `@brink/ink-operations`'s `ConvertibleShape`) stays in the editor.
 */
/**
...
function templateRoundtrips(re: RegExp, shape: PatternShape): boolean {
  ...
  return ROUNDTRIP_PROBES.some((probe) => {
    ...
    // The group the template's placeholder round-trips through is
    // `template_group` when a kind declares one (#406 — e.g. the at-cue
    // parenthetical's bare `content_inner`, while its `content_group` is the
    // wrap-inclusive `content`), else `content_group`. Mirrors Rust's
    // `validate_template_roundtrip`; checking `content_group` alone falsely
    // failed the shipped preset.
    const group = shape.template_group ?? shape.content_group;
    if (group) {
      return m.groups[group] === probe;
    }
    return true;
  });
}
...
export class ResolvedDialect {
  private constructor(
    readonly elements: ResolvedElement[],

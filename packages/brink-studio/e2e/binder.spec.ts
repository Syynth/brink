...
test.describe("binder → tab opening", () => {
  ...
  test("single-click on knot opens unpinned symbol tab", async ({ page }) => {
    ...
    await page.locator(".brink-binder-knot .brink-binder-label", { hasText: "start" }).click();
  ...
  test("single-click on different knot replaces unpinned tab", async ({ page }) => {
    ...
    // Click "story"
    await page.locator(".brink-binder-knot .brink-binder-label", { hasText: "story" }).click();
    await expect(page.locator(".brink-tab", { hasText: "story (main.ink)" })).toBeVisible({ timeout: 2000 });

    // "start" tab should be gone (replaced)
    await expect(page.locator(".brink-tab", { hasText: "start (main.ink)" })).toHaveCount(0);
...
test.describe("tab pinning", () => {
  ...
  test("pinned tab survives when another unpinned tab opens", async ({ page }) => {
    ...
    await expect(page.locator(".brink-tab", { hasText: "start (main.ink)" })).toBeVisible();
...
test.describe("focused view content", () => {
  ...
  test("knot tab shows only that knot's content", async ({ page }) => {
    ...
    expect(content).toContain("Hello, world!");
    expect(content).not.toContain("=== story ===");
    expect(content).not.toContain("// Welcome to brink studio!");
  });
  ...
  test("edits in focused view splice back correctly into full file", async ({ page }) => {
    ...
    // The focused view should contain "=== start ===" and end with a blank line before "=== story ==="
    const focusedContent = await getEditorContent(page);
    expect(focusedContent).toContain("=== start ===");
    expect(focusedContent).not.toContain("=== story ===");
    ...
    expect(fullContent).toContain("-> start");
    // The blank line between the start knot's last line and story knot should be gone.
    // getEditorContent returns text without newlines (CM renders lines as separate elements),
    // so "-> END" should be immediately followed by "=== story ===" with no blank space.
    expect(fullContent).toMatch(/-> END=== story ===/);
  });
  ...
  test("switching back to file tab shows full file", async ({ page }) => {
    ...
    // Switch back to file tab — click the label directly to avoid matching "start (main.ink)"
    await page.locator(".brink-tab .brink-tab-label", { hasText: /^main\.ink$/ }).click();
    await page.waitForTimeout(100);
    ...
    expect(content).toContain("=== start ===");
    expect(content).toContain("=== story ===");
    expect(content).toContain("// Welcome to brink studio!");
  });
});

...
- **The CLI (`brink`)** — `compile` an `.ink` story to bytecode, `play` it in the terminal, or `convert` between the binary (`.inkb`) and text (`.inkt`) story formats. Your entry point for everyday use.
...
The native compiler is still closing the last gap to full parity; the oracle corpus tracks exactly where it stands, and a ratchet test keeps every change byte-identical or better.

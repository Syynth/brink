...
- **Build the wasm package before `pnpm install --frozen-lockfile`, not after.** `@brink-lang/web` (`packages/wasm`) has a `file:` dependency on `crates/brink-web/www/pkg`. Every CI lane sequences correctly, but `pnpm install --frozen-lockfile` does NOT reliably fail loudly when the ordering is skipped locally — it can exit 0 with the link silently unresolved, and the resulting `preinstall`/`postinstall` skip means no pnpm lifecycle hook can catch it after the fact either (#2479). Run `wasm-pack build crates/brink-web --target web --out-dir www/pkg` first, then `pnpm install --frozen-lockfile`; `pnpm check:wasm-pkg` (`scripts/check-wasm-pkg.mjs`) runs two independent, fast, explicit checks for this failure — `checkWasmPkg` (does `crates/brink-web/www/pkg` hold a built wasm-pack output?) and `checkWasmPkgLink` (did that output actually resolve into `packages/wasm/node_modules/brink-web`? #2514) — with different remediations (build wasm vs. reinstall); run it right after install if you skipped the ordering or are unsure. This ordering is CI-self-enforcing (#2504): `every_pnpm_install_lane_builds_wasm_first_in_the_same_job` (`packages/brink-desktop/src-tauri/src/lib.rs`) parses every job in every `.github/workflows/*.yml` file and fails if any `pnpm install --frozen-lockfile` step lacks a preceding `wasm-pack build crates/brink-web` step in the same job — a reordered or newly-added lane fails this test rather than silently re-opening #2479.

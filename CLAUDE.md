...
2. **One engine, several pulls** (option A total, ruled 2026-08-24 — this REVISES the former "Both roads" doctrine: the off-db snapshot road, `IdeSnapshot::analyze` → `analyze_with_modules`, is DELETED). All analysis is `ProjectDb`'s salsa composition; the surfaces that matter are distinct *pulls* of it — the whole-project `analysis_query` bundle (`session.analysis()`), the per-file `diagnostics_query` road (Problems panel; also carries parse/lowering diagnostics), and the member-set `subset_analysis_query` (brink-lsp's per-root road). A change can still wire one pull wrong while another stays right — exercise the pulls a change touches, and remember a green `brink compile` is NOT evidence the editor surfaces agree.
...
On a fresh checkout (including cloud sessions with no local toolchain cache), run `scripts/setup-dev.sh` first. It installs/verifies rustup + the pinned toolchain (`rust-toolchain.toml`), `wasm-pack`, and `pnpm` (via corepack), mirroring the versions `.github/workflows/ci.yml` uses. Setting `BRINK_SETUP_FULL=1` additionally installs CI's pinned `cargo-deny` and runs both workspace audits (root and `packages/brink-desktop/src-tauri`) — opt-in because cargo-deny has no prebuilt binary and compiles from source in ~2m, which is real latency at every cloud-session start for gates CI already enforces. Setting `BRINK_SETUP_FRONTEND=1` additionally builds **every** `file:`-linked wasm-pack output in `scripts/check-wasm-pkg.mjs`'s `WASM_PACKAGES` registry (`crates/brink-web` and `crates/brink-prose` — the script reads the list from the registry, so a third crate inherits it) and then runs `pnpm install:checked -- --frozen-lockfile`, each step timeout-bounded and **fatal** on failure. That is the stage a **cloud-environment setup script** needs: the two-liner `bash scripts/setup-dev.sh` + `pnpm install --frozen-lockfile` dies at the second line with `ENOENT … scandir crates/brink-prose/www/pkg` (nothing built the wasm first), and a non-zero setup script means the session never starts. The setup script for the environment is therefore the single line `BRINK_SETUP_FRONTEND=1 bash scripts/setup-dev.sh` (after `cd` to the checkout). Opt-in because the cold wasm builds are the slowest thing the script can do (measured cold on a 4-job cloud VM: 2m43s for brink-web, ~8m30s for brink-prose, wasm-opt included) and a Rust-only session has no use for them; note the web docs ask setup scripts to finish in roughly five minutes so the environment snapshot can build, and this stage alone runs past that on a cold cache.
...
- **Build the wasm packages before installing, and install via `pnpm install:checked`.** `@brink-lang/web` (`packages/wasm`) has a `file:` dependency on `crates/brink-web/www/pkg`, and `@brink-lang/studio` (`packages/brink-studio`) has a SECOND one on `crates/brink-prose/www/pkg` (the prose checker, #3208) — both are registered in `scripts/check-wasm-pkg.mjs`'s `WASM_PACKAGES`, and `install:checked` refuses until every registered output is built, naming each missing build. Every CI lane sequences correctly, but a bare `pnpm install --frozen-lockfile` does NOT reliably fail loudly when the ordering is skipped locally, and it has been observed failing in **two different shapes** for the same skipped ordering: exiting 0 with the link silently unresolved (#2479), and writing **no `node_modules` at all** while the only visible symptom was a bare `vitest: not found` from the *next* command (#2593). Which shape a machine got used to depend on the pnpm 10.x corepack resolved there, because the repo pinned only the major; **#2604 pins an exact version** (root `package.json`'s `packageManager` field — the single source `scripts/setup-dev.sh` and every `pnpm/action-setup` lane derive from, enforced by `scripts/check-pnpm-pin.mjs` via `pnpm test:scripts`). That makes the shape reproducible, not harmless — **never rely on `pnpm install`'s exit code to tell you the install happened.** No pnpm lifecycle hook can cover the gap either: pnpm skips every project lifecycle script when a per-package link fails, so a root `preinstall` is dead code in exactly this state (verified by direct probe on pnpm 10.34.5 for #2593, and earlier for #2479).
  ...
  - Most CI lanes need neither: the ordering is CI-self-enforcing (#2504) via `every_pnpm_install_lane_builds_wasm_first_in_the_same_job` (`packages/brink-desktop/src-tauri/src/lib.rs`), which parses every job in every `.github/workflows/*.yml` file and fails if any `pnpm install --frozen-lockfile` step lacks a preceding `wasm-pack build crates/brink-web` step in the same job. That test guards the lanes it can see in workflow YAML — but `.github/workflows/book.yml` runs `just book-assets`, whose recipe body invokes `pnpm install:checked` from inside a `justfile` recipe rather than a literal workflow step, so the YAML parser has no visibility into it. `book-assets` is guarded precisely because of that blind spot; `install:checked` covers both the local path the CI-self-enforcing test cannot see and this one CI lane it cannot see either. That guard then FIRED, for nine days, and nothing was watching it: #3208 registered `crates/brink-prose` as a second `WASM_PACKAGES` entry, every consumer that READS the registry picked it up, and the `just wasm`/`book-assets` recipes — which named `brink-web` literally — did not, so `install:checked` correctly refused and `book.yml` went red on every push to `main`. Both recipes now derive their crate list from the registry the way `scripts/setup-dev.sh` already did, and `book-assets` takes `wasm` as a dependency rather than carrying a second hand-written copy of the build, so a third crate inherits both. The standing lesson is narrower than "add the crate": a hand-maintained restatement of that registry is the drift, wherever it lives.
...
| `benchmarks/drivers/brink-loop/**` | `cd benchmarks/drivers/brink-loop && cargo fmt -- --check && cargo clippy --all-targets -- -D warnings && cargo build --release` | ⚠ Its own cargo workspace, so root `cargo fmt --all`/`clippy --workspace` never see it; `benchmarks.yml`'s `BENCHMARKS_BRINK_LOOP_GATE` does (it went red on #3575 exactly this way). |
| **`crates/internal/brink-db/src/queries/**`** | `cargo test -p brink-db` — and read `tests/query_execution_counts.rs`'s failure, not just its verdict | ⚠ A query with a too-COARSE dependency is a *correct* query: right value, every correctness test green, and the only thing it changes is how often it re-executes. `query_execution_counts.rs` pins **exact** per-query execution counts after each edit shape (edit inside a knot, append, `VAR` initializer, other file, insert a knot) via `brink_db::count_executions`. If it moves, you widened or narrowed a dependency — the number names which query. It went 0-for-1 on reading: the per-def inference family's per-knot firewall was defeated by a `Vec` index no code review saw, and the counter found it in one screen (#3585). |
...
| `brink-opt` | `crates/brink-opt/` | Post-compile `.inkb` → `.inkb` optimizer: the peephole engine + superinstruction passes (`docs/optimizer-peephole.md`); `brink-format`-only dep, `publish = false` |
...
- **A salsa dependency is verified by counting executions, not by reading the query.** Too coarse is the safe-looking mistake: correct output, green tests, quadratic re-execution (505 ms per keystroke at 480 knots, #3585). `brink_db::count_executions(|| …)` returns `{query → executions}` for the closure, fed by salsa's `WillExecute` event (validations and backdates do not count; thread-local; off by default). Any change to a `brink-db` query's inputs — including reaching a tracked struct through a `Vec` index, which makes every reader depend on every element — needs a pinned count in `crates/internal/brink-db/tests/query_execution_counts.rs`, and the pin is exact, like `db_memo_retention`'s. Two salsa facts that cost a day each, recorded here so they cost no one another: a `(project, file, tracked-struct)`-keyed query interns one key per re-minted struct and its LRU turns over too slowly at one key per edit to pass `db_memo_retention` (make it a plain function unless it has real traffic); and a tuple carrying a tracked struct has no `salsa::Update`, so return `'static` indices and go through a per-index tracked accessor.

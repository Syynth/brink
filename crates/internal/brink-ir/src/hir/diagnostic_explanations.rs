...
This is checked over the handler's **transitive** call closure, not just its own direct calls: `handler() -> helper() -> get_health()` is diagnosed even though `handler` never calls `get_health` itself — the diagnostic is anchored at the real offending call site (inside `helper`'s own body here), which may be in a different definition, and a different file, than the handler's own declaration."#,
    ...
    (
        DiagnosticCode::E183,
        r#"`lower_call` (`brink-ir::lir::lower::expr`) turns a resolved call-site symbol into LIR. Most resolved kinds have an explicit lowering (`External` → `CallExternal`, `List` → a list-conversion builtin, `Variable`/`Constant` → `CallVariable`, `Knot` → `Call` — ink allows any knot as a function via tunnels, so no other kind needs its own arm to be *legitimately* callable). Every other kind reaching this point is not callable, and this code refuses it with a diagnostic instead of falling through a catch-all that would emit `lir::Expr::Call` against whatever id happens to be resolved there.

...
/// Returns true if the case has no usable ink source (empty story.ink).
/// Some cases are dynamically generated by the C# test suite and cannot
/// be extracted as static `.ink` files.
...
fn corpus_report() {
    ...
    for case_dir in &cases {
        ...
        let ink_path = case_dir.join("story.ink");
        if !ink_path.exists() || has_empty_source(case_dir) {
            cat.cases_skip += 1;
            continue;
        }
        ...
        let (_story_data, actual) = match compile_and_explore_from_ink(&ink_path, &config) {
            Ok(pair) => {
                if expect_compile_error {
                    // Compilation succeeded but should have failed — skip.
            ...
            Err(_) if expect_compile_error => {
                // Compilation failed as expected — skip.
                cat.cases_skip += 1;
                continue;
            }

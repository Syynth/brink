//! Compile-checks the "The Numeric Tower" book chapter's `ink`/`text`
//! example pairs (issue #1184).
//!
//! Same shape as `book_path_projections_examples.rs`: the book's standing rule
//! ("Book code examples must be compile-checked") has no `mdbook test`
//! equivalent for `ink` fences, so this test parses
//! `docs/book/src/toolchain/dialect/numeric-tower.md`'s fenced code
...
#[test]
fn every_ink_example_in_the_numeric_tower_chapter_compiles_and_matches_its_output() {
    let markdown = std::fs::read_to_string(chapter_path()).expect("read numeric-tower.md");
    ...
    assert!(
        ink_example_count >= 10,
        "expected the chapter to have multiple worked ink examples \
         covering vectors, quaternions, matrices, and scalar kit — found {ink_example_count}"
    );
}

...
fn runtime_function_with_whitespace_only_template_trimmed() {
    ...
    // The template resolves to just " " (whitespace). trim_function_end
    // should resolve the LineRef and remove it, matching C#'s behavior.
    let source = "\
VAR globalVal = 5
{globalVal}
...
";
    ...
    assert_eq!(
        text2, "625\n",
        "squaresquare's whitespace-only template should be trimmed; \

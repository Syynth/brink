//! Corpus round-trip test — port of `brink-syntax`'s
//! `tests/corpus_roundtrip.rs` to the native `.brink` grammar (#1274, from
//! #1199 / #1256).
//!
//! For every `.brink` file under this crate's `tests/corpus/` directory,
//! parse it and assert the CST reprints byte-for-byte (losslessness) with
//! no panic. This is a syntax-layer invariant only — no HIR lowering, no
//! oracle comparison, off the ink pipeline entirely.

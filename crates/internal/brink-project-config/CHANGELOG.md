...
## [0.0.17](https://github.com/Syynth/brink/compare/brink-project-config-v0.0.16...brink-project-config-v0.0.17) - 2026-09-12

### Added

- *(project-config)* scalar readers, set_bool and the key list on ConfigDocument
- *(gpui)* brink.toml in the shared buffer, and Settings ▸ General over it
- *(brink-project-config,studio)* [fix] table, effective_fix_policy, Fix column
- project-declared dialogue dialect — brink.toml [dialogue], default none ([#3387](https://github.com/Syynth/brink/pull/3387))
- *(studio,editor,web)* prose dictionary lives in brink.toml, with a settings view
- *(config)* [prose] table in brink.toml — dialect and enable
- `[project] drafts` — declare deliberately-unfinished files ([#3145](https://github.com/Syynth/brink/pull/3145)) ([#3155](https://github.com/Syynth/brink/pull/3155))
- *(config)* `[project] indent` in brink.toml
- *(config)* a round-trip-preserving writer for brink.toml

## [0.0.15](https://github.com/Syynth/brink/compare/brink-project-config-v0.0.11...brink-project-config-v0.0.15) - 2026-08-23

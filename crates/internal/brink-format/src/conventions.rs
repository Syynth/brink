...
//!
//! # `transitions`/`templates` do not live here (2026-08-05 ruling)
//!
//! Issue #2115 briefly gave this shape `transitions`/`templates` fields —
//! `brink_ir::dialect::DialogueDialect`'s editor-overlay succession rows
//! (Tab/Enter/Shift-Tab behavior, picker labels), carried verbatim. The
//! 2026-08-05 ruling *"Succession is EDITOR-OWNED and externally defined"*
//! (`docs/decision-log.md`, PR #2276) reversed that transport: `dialect.rs`
//! already says these rows "never travel beyond tooling", and `.inkb` **is**
//! beyond tooling — it is the compiled artifact a game host loads, and a
//! runtime has no Tab key. Issue #2277 stripped them back out of this wire
//! shape and its codec. The editor now supplies succession rows externally;
//! `brink_ir::ConventionsProjection::with_succession` and
//! `brink_ir::dialect::validate_succession` still re-key those
//! externally-supplied rows against this projection's real convention kinds
//! in-process (so a rule naming a nonexistent kind fails loudly) — that
//! validator half of #2115 survives the ruling intact. Only the wire
//! transport half was undone.
...
/// `transitions`/`templates`), then `2` → `3` by issue #2277, which removed
/// them again per the 2026-08-05 ruling — see this module's own doc.
/// Nothing has emitted any of these versions into a real `.inkb`/`StoryData`
/// yet (see this module's own doc — the wire shape is not wired into
/// `crate::StoryData` at all), so there is no on-disk payload either bump
/// could orphan; the version byte exists so a future reader can tell the
/// shapes apart the moment this section *is* wired in, rather than needing
/// an out-of-band note.

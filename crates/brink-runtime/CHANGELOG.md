...
## [0.0.17](https://github.com/Syynth/brink/compare/brink-runtime-v0.0.16...brink-runtime-v0.0.17) - 2026-09-12
...
- *(runtime)* `<-` pushes no call frame, so thread loops stay bounded ([#3561](https://github.com/Syynth/brink/pull/3561))
...
- *(runtime)* build an empty origin list without the `vec!` macro
...
- bind parameters at the function-value entry site too
- stop allocating per call in the parameter binding path
- bind container parameters at entry, not by a bytecode prologue
- add the left-operand-fold superinstructions, bump .inkb to v9
- add the binary-fusion superinstructions, bump .inkb to v8
- Merge remote-tracking branch 'origin/main' into opt/peephole-emit-line-nl
- Merge pull request #3575 from Syynth/bench/opcode-histogram
- Merge pull request #3574 from Syynth/perf/linker-global-rewrite
- Merge pull request #3573 from Syynth/perf/linker-target-rewrite
- *(runtime)* resolve static jump/call targets once at link time
- *(runtime)* build each delivered line in place instead of through per-part strings
- *(runtime)* lay each thread's call stack out contiguously
- *(runtime)* build a RuntimeError only on the None arm — ok_or_else everywhere
- *(runtime)* track line completion incrementally instead of rescanning per step
- *(runtime)* CallStack is a plain Vec — drop the Arc prefix and snapshot cache
- Merge remote-tracking branch 'origin/main' into fix/3561-thread-frame
- Merge remote-tracking branch 'origin/main' into fix/3561-thread-frame

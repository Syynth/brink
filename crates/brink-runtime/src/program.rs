...
pub struct Program {
    ...
    /// D6 (`docs/debugger-spec.md` §2, `.inkb` tag `0x11`) `DebugInfo`
    /// section, carried through unmodified from `StoryData::debug_info`.
    /// `None` for a release-exported artifact (§1.2's ship policy — never
    /// requested) or any story compiled before D6. D7 (#3185) is the first
    /// consumer: [`crate::debug::DebugFrame::locals`] resolves a frame's
    /// currently-open `container_stack` against `containers[i].locals`
    /// (§3's per-container `LocalsTable`) to name its live temp slots.

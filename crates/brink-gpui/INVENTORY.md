...
| Per-LINE styles: cue lines, dimmed comment/include lines | half built 2026-09-07 | **dimmed lines are in**: an `INCLUDE`/`EXTERNAL` line and a whole-line comment fade to `MUTED_FADE`, keeping their own colours so a comment stays comment-coloured and a keyword stays a keyword — they simply stop competing with the prose. Derived from the SOURCE (both shapes are decidable from the text), so the paint does not lag the keystroke. **Cue lines are not**: those need the dialect classification `IdeSession::line_contexts` carries, which the worker does not expose per file — a real port, and one that has to decide how an async classification reaches a synchronous highlighter. |

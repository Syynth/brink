...
pub fn run(story: &mut Story, config: TuiConfig) -> Result<(), Box<dyn std::error::Error>> {
    let mut terminal = ratatui::init();
    let result = run_inner(&mut terminal, story, config);
    ratatui::restore();
    result
}

...
use super::model::{ScenarioConfig, ScenarioResult, TurnWeight, run_batch_scenario, run_scenario};
...
/// (issue #900's baselines) or BH-2's batch driver (`advance_batch`,
/// issue #914). Selected with `--mode serial|batch`; each mode writes its
/// own baseline file pair so regenerating one never clobbers the other.
...
fn parse_cli() -> Result<(usize, DriverMode), Box<dyn std::error::Error>> {
    ...
    for w in args.windows(2) {
        ...
        if w[0] == "--mode" {
            mode = match w[1].as_str() {
                ...
                other => {
                    return Err(format!("unknown --mode `{other}` (expected serial|batch)").into());
                }
            };
        }
    }
...
/// prefix (`serial-*` / `batch-*`) so the two baseline files stay
/// self-describing.
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    let (frames, mode) = parse_cli()?;
    let label = mode.label();
    println!(
        "scenario_bench | frames={frames} | mode={label} | headless MinimalPlugins runner (issue #900 / #914)"
    );
    println!(

...
const PARALLEL_MD_PREAMBLE: &str = "Generated by `cargo bench -p bevy-brink --bench scenario_bench -- --mode parallel`. \
     ...
     pass, `collect p50` reads 0. Expect parity-or-worse at low flow counts \
     (task-spawn overhead) and the win to appear as Step outgrows the fixed \
     per-turn cost. Since #937 there is no per-flow frame-start clone: each \
     flow borrows the frame-start world (§12.2 \"borrow, don't copy\") and \
     overlays its own writes, so the fixed per-turn cost no longer scales with \
     story-world size — raise `--story-globals N` to see that axis at all. \
     The `ComputeTaskPool` thread count is process-global and \
...
const BATCH_MD_PREAMBLE: &str = "Generated by `cargo bench -p bevy-brink --bench scenario_bench -- --mode batch`. \
     ...
     measurable from outside the driver). BH-2's per-flow frame-start snapshot \
     clone is gone as of #937: Step now borrows the frame-start world and \
     overlays each flow's own writes (§12.2 \"borrow, don't copy\"), so the \
     per-flow cost no longer scales with story-world size — raise \
     `--story-globals N` to see that axis at all. See \

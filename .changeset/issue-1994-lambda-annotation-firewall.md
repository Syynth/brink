...
- A body-derived type that disagrees with the lambda's own written annotation is now reported as a new diagnostic, **`E173`**, raised eagerly at the lambda's own declaration — not deferred to wherever the lambda is later called. Unlike the gradual/advisory `E063`, `E173` is `Error`-severity by default and not `[lints]`-downgradable.

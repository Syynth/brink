...
ALSO: note in \`scopeGaps\` anything the issue (or its milestone) UNDER-captured — work that clearly belongs but no issue covers. Return {pr, decision, findings, scopeGaps}.`,
...
${TRAIN_SETUP}
REVIEWER FINDINGS (if these read as placeholders/non-actionable, verify against the LIVE PR — gh api reviews/comments — and if nothing real exists, treat as nothing-to-fix and proceed straight to the land steps):\n- ${(base.verdict.findings ?? []).join("\n- ")}
